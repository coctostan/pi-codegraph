...
### Fixed
- **impact: traverse `implements` edges and emit informative empty-hit diagnostics (#073, #074):** `collectImpactDetails` BFS now merges inbound `implements` edges alongside `calls` edges, so changing an interface correctly surfaces all implementing classes (breaking, depth:1) and their callers (behavioral, depth:2+). Deduplication handles nodes that both implement and call the same seed. `impact()` replaces the silent empty-body return with a per-seed diagnostic distinguishing entry-points (`No dependents found — '<name>' is an entry point with no callers.`), interfaces with no dependents (`No call-edge dependents found for interface '<name>'. Consider checking implementors via symbol_graph.`), and genuinely isolated symbols. Both fixes are contained in `src/tools/impact.ts`; no graph or store changes required.
- **Interface contract output and false `implements` edges (#076, #077):** `symbol_graph(include:["contract"])` on interface symbols now renders `### Methods` and `### Fields` sections with the full member list instead of the broken `### Takes\n  interface Foo` output. Fixed by (1) teaching `extractInterfaceSignature` to walk the interface body AST and serialize members into the stored signature, and (2) adding interface-aware rendering in `renderSymbolContractBody` that parses the stored multiline signature (with source-file fallback). Separately, `resolveImplementations` no longer persists a false `implements` edge when the LSP returns an in-method location (e.g. a return-type usage) for a data-only interface — the LSP-success path now validates the class `implements` clause via `classImplementsInterface()` before writing any edge, matching the strictness of the existing fallback path. (#084)
...
- **trace: repair class fallback and symbol lookup messaging (#085):** class entries no longer render as `[leaf]` in static mode; `trace()` now emits a class-specific redirect for symbols like `SqliteGraphStore` and `BM25Index` instead of pretending they are terminal steps. Missing lookups now use `Symbol` wording, and file-filter misses retry an unscoped lookup so output surfaces real candidate file locations instead of returning the generic `Entry ... not found` message. Adds reproductions for the class-entry and file-filter-miss cases while preserving existing static-fallback and ambiguity behavior.
...
### Added
- Optional `suppressTrustHeader` boolean on `symbol_graph`, `impact`, and `trace`: when `true`, omits the `## Trust` header block from tool output for follow-up calls in multi-call sessions; default behavior (omitted / `undefined` / `false`) is byte-identical to the pre-change baseline; suppression is centralized in `finalizeReadOnlyOutput` and leaves the `indexing-failed` note, `_meta` footer, anchors, and signals intact (#075)
